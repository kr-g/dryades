...
class HeapTestCase(unittest.TestCase):
    ...
    def test_append_out_of_memory(self):
        ...
        # open with max_size limit
        hpf = HeapFile(fnam, max_size=0x100).open()

        node = hpf.alloc_append(0x0F)
        node2 = hpf.alloc_append(0x20)

        self.assertRaises(Exception, hpf.alloc_append, (0xC0), msg="out of memory")
